...
*/
	...
	var template = {
		...
		categoryLIST : function(item){
			...
			{
				list = list + `<div data-index="${i}" style="background: url('${item[i].bg}');"><img src="${item[i].img}"></div>`;
			}					
		...
		selectedITEMS : function(name){
			...
			{
				arr = this.selectedItem; console.log('222arr',arr);
			}
	...
	function setSelectedAllItemsPage(name){
		return template.selectedITEMS(name);
	}
