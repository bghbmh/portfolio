			...
			on: {
				'draw.ft.table': function (e, ft) {
					if ($(this).find('.footable-empty').size() > 0) {
						...
						$(this)
							.find('.footable-empty')
							.append('<td colspan=' + $(this).find('th').size() + '>등록된 내용이 없습니다.</td>');
					}
				},
			},
			breakpoints: {
...
function controller() {
	...
	//message_box 닫기 버튼
	$('.dash_btn_box .btn_close').on('click', function () {
