...
export var Sample = (function() {
	...
	return {
		setItems: function( selectedCategory = null ){
			...
			if( selectedCategory ){
				if( _category.findIndex( v => v ===  selectedCategory) >= 0 ) {
					_itemsBox.innerHTML = "";
		...
		setNav: function(){ console.log("setNav");
			...
			/* 임시 */
			//_body.insertBefore(_nav, _body.firstChild);
		},
		...
		sampleHtml: function (){
			...
			if( !_selectedCategory ) return console.log("no _selectedCategory");
		...
		setState: function(clickMenu){
			...
			console.log("setStatel00 ================",);

			let c = clickMenu ? clickMenu.dataset.category : url.searchParams.get("category");

			// if( c )
			// 	url.resetState("category", c , { page_id: 1, user_id: 5 } );

			if( !clickMenu && c ){
				url.resetState("category", c , { page_id: 1, user_id: 5 } );
				this.setItems(c);
			 return console.log("list? clickMenu - ", clickMenu);
			}
			...
			url.resetState("category", clickMenu.dataset.category , { page_id: 1, user_id: 5 } );
			this.setItems(clickMenu.dataset.category);
		},
